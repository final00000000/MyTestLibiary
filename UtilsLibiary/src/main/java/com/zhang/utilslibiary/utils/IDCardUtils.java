...
public final class IDCardUtils {
    ...
    public static boolean validateIdCard15(final String idCard) {
        ...
        if (isNumber(idCard) && idCard.length() == CHINA_ID_MIN_LENGTH) {
            ...
            Date birthDate = null;
            try {
                birthDate = DateUtil.INSTANCE.getSafeDateFormat("yy-MM-dd").parse(birthCode.substring(0, 2));
            } catch (ParseException e) {
                Timber.e("IDCardUtils_134行_2022/7/27_14:59：%s", e.getErrorOffset());
            }
        ...
        if (idCard != null && idCard.length() == CHINA_ID_MAX_LENGTH) {
            ...
            if (isNumber(code17)) {
                try {
                    ...
                    Timber.e("IDCardUtils_173行_2022/7/27_15:00：%s", e.getMessage());
                }
            }
        }
        return false;
    ...
    public static String convert15CardTo18(final String idCard) {
        ...
        if (isNumber(idCard) && idCard.length() == CHINA_ID_MIN_LENGTH) {
            ...
            try {
                birthDate = DateUtil.INSTANCE.getSafeDateFormat("yyyy-MM-dd").parse(birthday);
            } catch (ParseException e) {
            }
            Calendar calendar = Calendar.getInstance();
            if (birthDate != null) calendar.setTime(birthDate);
