package com.zhang.utilslibiary.utils.verification
...
import com.zhang.utilslibiary.utils.DisplayUtils
import com.zhang.utilslibiary.utils.DisplayUtils.px2dip
...
class DragImageView : FrameLayout, SeekBar.OnSeekBarChangeListener {
    ...
    private var swipeVerifi: SwipeCaptchaView? = null
...
//        sb!!.max = 10000
    ...
    fun setUp(cover: Bitmap, block: Bitmap) {
        ...
        l.width = DisplayUtils.dip2px(w.toFloat())
        l.height = DisplayUtils.dip2px(h.toFloat())

        drag_iv_cover!!.layoutParams = l
        drag_iv_cover!!.setImageBitmap(cover)
        val l2 = drag_iv_block!!.layoutParams as MarginLayoutParams
        l2.height = DisplayUtils.dip2px(block.height.toFloat())
        l2.width = DisplayUtils.dip2px(block.width.toFloat())
    ...
    override fun onProgressChanged(seekBar: SeekBar, progress: Int, fromUser: Boolean) {
        pro = progress
        swipeVerifi!!.setCurrentSwipeValue(progress)

        verifiRest()
        val cw = drag_iv_cover!!.measuredWidth
        val bw = drag_iv_block!!.measuredWidth
        val l = drag_iv_block!!.layoutParams as MarginLayoutParams
...
//        SwipeVerifi.max = mSwipeCaptchaView.getMaxSwipeValue()

        //随便放这里是因为控件
        sb!!.max = swipeVerifi!!.maxSwipeValue
        sb!!.progressDrawable = context.resources.getDrawable(R.drawable.drag_seek_progress)
        drag_iv_block!!.visibility = View.VISIBLE
        drag_iv_cover!!.setImageBitmap(cover)
...
//        drag_v_flash!!.visibility = View.VISIBLE
    ...
    fun SwipeVerifi(swipeVerifi: SwipeCaptchaView) {
        this.swipeVerifi = swipeVerifi
